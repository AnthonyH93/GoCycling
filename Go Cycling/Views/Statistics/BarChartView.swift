...
struct BarChartView: View {
                ...
                VStack {
                    HStack {
                        ForEach (0..<chartViewModel.pastData[index + (barChartUnitsSelection.id * 3)].count, id: \.self) { id in
                            BarChartCellView(value: chartViewModel.pastDataNormalized[index + (barChartUnitsSelection.id * 3)][id], barColor: Color(UserPreferences.convertColourChoiceToUIColor(colour: newPreferences.colourChoiceConverted)))
    ...
    func updateCurrentValue()    {
        ...
        switch (barChartUnitsSelection) {
        case .distance:
            selectedValue = "\(MetricsFormatting.formatDistance(distance: chartViewModel.pastData[index + (barChartUnitsSelection.id * 3)][id], usingMetric: newPreferences.usingMetric))"
        case .time:
