...
struct CyclingRecordsView: View {
    ...
    var body: some View {
        Section(header: Text(RecordsFormatting.headerStrings[0]), footer: Text(RecordsFormatting.getCyclingRecordsFooterText(usingMetric: preferences.storedPreferences[0].usingMetric))) {
            VStack {
                HStack {
                ...
                CyclingSingleRecordView(recordValue: MetricsFormatting.formatDistance(distance: records.storedRecords[0].totalCyclingDistance, usingMetric: preferences.storedPreferences[0].usingMetric), recordName: RecordsFormatting.recordsStrings[0], recordDate: nil, firstEntry: true)
