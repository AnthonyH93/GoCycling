...
struct MapWithSpeedView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                if (newPreferences.largeMetrics) {
                    ...
                    ZStack {
                        if (mapCentered) {
                            Button (action: {self.toggleMapCentered()}) {
                                MapSystemImageButton(systemImageString: "lock", buttonColour: (UserPreferences.convertColourChoiceToUIColor(colour: newPreferences.colourChoiceConverted)))
                                    .padding(.bottom, 5)
                                }
                        }
                        else {
                            Button (action: {self.toggleMapCentered()}) {
                                MapSystemImageButton(systemImageString: "lock.open", buttonColour: (UserPreferences.convertColourChoiceToUIColor(colour: newPreferences.colourChoiceConverted)))
                                    .padding(.bottom, 5)
                                }
                        }
                    }
